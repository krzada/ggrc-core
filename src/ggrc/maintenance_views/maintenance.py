...
def get_latest_task_status(bg_name):
  ...
  return BackgroundTask.query.filter(BackgroundTask.name.contains(bg_name)) \
                             .order_by('-id') \
                             .first() \
                             .status
...
def set_latest_task_status(bg_name):
  ...
  if status == BackgroundTask.PENDING_STATUS:
    return "Not started"
  elif status == BackgroundTask.RUNNING_STATUS:
    return "In progress"
